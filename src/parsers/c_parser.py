"""Parse C / C++ source → unified AST using tree-sitter-c or tree-sitter-cpp."""
...
_NODE_IMPORT       = "preproc_include"
_NODE_FUNCTION     = "function_definition"
_NODE_METHOD       = "function_definition"   # C++ member functions
_NODE_CLASS        = "class_specifier"       # C++ only
_NODE_STRUCT       = "struct_specifier"
_NODE_INTERFACE    = "class_specifier"
_NODE_VAR          = "declaration"
_NODE_CONST        = "declaration"           # distinguish by type_qualifier

_STMT_RETURN       = "return_statement"
_STMT_IF           = "if_statement"
_STMT_WHILE        = "while_statement"
_STMT_FOR          = "for_statement"
_STMT_FOR_EACH     = "for_range_loop"        # C++ range-for
_STMT_ASSIGN       = "assignment_expression"
_STMT_VAR_DECL     = "declaration"
_STMT_EXPR         = "expression_statement"
_STMT_BREAK        = "break_statement"
_STMT_CONTINUE     = "continue_statement"

_EXPR_CALL         = "call_expression"
_EXPR_BINARY       = "binary_expression"
_EXPR_UNARY        = "unary_expression"
_EXPR_FIELD        = "field_expression"      # obj.field  (also pointer_expression for ->)
_EXPR_INDEX        = "subscript_expression"
_EXPR_IDENT        = "identifier"
_EXPR_INT_LIT      = "number_literal"
_EXPR_FLOAT_LIT    = "number_literal"
_EXPR_STRING_LIT   = "string_literal"
_EXPR_BOOL_TRUE    = "true"
_EXPR_BOOL_FALSE   = "false"
_EXPR_NULL         = "null"                  # NULL / nullptr both map here

_IDENT_FIELD       = "name"

_PRIM_TYPES: Dict[str, object] = {
    "char":               lambda: T_STRING,
    "signed char":        lambda: T_NUMBER(8, signed=True),
    "unsigned char":      lambda: T_NUMBER(8, signed=False),
    "short":              lambda: T_NUMBER(16, signed=True),
    "unsigned short":     lambda: T_NUMBER(16, signed=False),
    "int":                lambda: T_NUMBER(32, signed=True),
    "unsigned int":       lambda: T_NUMBER(32, signed=False),
    "long":               lambda: T_NUMBER(64, signed=True),
    "unsigned long":      lambda: T_NUMBER(64, signed=False),
    "long long":          lambda: T_NUMBER(64, signed=True),
    "unsigned long long": lambda: T_NUMBER(64, signed=False),
    "float":              lambda: T_NUMBER(32, signed=True, float=True),
    "double":             lambda: T_NUMBER(64, signed=True, float=True),
    "long double":        lambda: T_NUMBER(128, signed=True, float=True),
    "bool":               lambda: T_BOOLEAN,
    "_Bool":              lambda: T_BOOLEAN,
    "void":               lambda: T_VOID,
    "size_t":             lambda: T_NUMBER(64, signed=False),
    "int8_t":             lambda: T_NUMBER(8, signed=True),
    "int16_t":            lambda: T_NUMBER(16, signed=True),
    "int32_t":            lambda: T_NUMBER(32, signed=True),
    "int64_t":            lambda: T_NUMBER(64, signed=True),
    "uint8_t":            lambda: T_NUMBER(8, signed=False),
    "uint16_t":           lambda: T_NUMBER(16, signed=False),
    "uint32_t":           lambda: T_NUMBER(32, signed=False),
    "uint64_t":           lambda: T_NUMBER(64, signed=False),
    "std::string":        lambda: T_STRING,
    "string":             lambda: T_STRING,
}

_LANG = "c"   # overridden to "cpp" in parse() when _IS_CPP is True
...
def parse(source: str, filename: str = "<string>") -> Module:
    if not _TS_AVAILABLE:
        raise RuntimeError("Neither tree-sitter-c nor tree-sitter-cpp is installed.")
    lang = "cpp" if _IS_CPP else "c"
    ...
    return Module(source_language=lang, source_file=filename, nodes=nodes)
...
def _parse_top(node: "Node", source: str) -> "Optional[ASTNode | List[ASTNode]]":
    ...
    if t == _NODE_STRUCT:
        return _parse_typedef(node, source, TypeDefCategory.STRUCT)
    if t == "class_specifier":
        return _parse_typedef(node, source, TypeDefCategory.CLASS)
    if t == "enum_specifier":
        return _parse_typedef(node, source, TypeDefCategory.ENUM)
    if t == "type_definition":      # typedef struct { } Foo;
        inner = _child(node, "struct_specifier", "enum_specifier")
        if inner:
            return _parse_typedef(inner, source, TypeDefCategory.STRUCT)
    if t in ("namespace_definition",):   # C++
        return None   # TODO: could recurse into namespace body
    if t == _NODE_VAR:
        return _parse_variable(node, source, is_const=False)
    if t in ("preproc_def", "preproc_function_def", "preproc_ifdef",
             "preproc_if", "preproc_elif", "preproc_else", "comment"):
        return None
    return None
...
def _parse_import(node: "Node", source: str) -> "List[ImportNode]":
    ...
    path_node = _child(node, "system_lib_string", "string_literal")
    module = _text(path_node, source).strip('<>"') if path_node else _text(node, source)
    idx = _IMPORT_CTR[0]; _IMPORT_CTR[0] += 1
    return [ImportNode(id=f"import:{idx}:{module}", module=module)]
