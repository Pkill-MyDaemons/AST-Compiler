"""Parse Zig source → unified AST using tree-sitter-zig."""
...
# ---------------------------------------------------------------------------
try:
    import tree_sitter_zig as _ts_lang
...
_NODE_IMPORT       = "variable_declaration"  # const std = @import("std"); treated as var
_NODE_FUNCTION     = "function_declaration"
_NODE_METHOD       = "function_declaration"
_NODE_CLASS        = "container_declaration" # struct / union / enum
_NODE_STRUCT       = "container_declaration"
_NODE_INTERFACE    = "container_declaration"
_NODE_VAR          = "variable_declaration"
_NODE_CONST        = "variable_declaration"  # distinguish by "const" vs "var" keyword

_STMT_RETURN       = "return_statement"      # actually @return in zig — "break" / fallthrough
...
_EXPR_NULL         = "null_literal"          # null / undefined in Zig
...
_PRIM_TYPES: Dict[str, object] = {
    ...
    "f16":   lambda: T_NUMBER(16, signed=True, float=True),
    "f32":   lambda: T_NUMBER(32, signed=True, float=True),
    "f64":   lambda: T_NUMBER(64, signed=True, float=True),
    "f128":  lambda: T_NUMBER(128, signed=True, float=True),
    "bool":  lambda: T_BOOLEAN,
    "void":  lambda: T_VOID,
    "type":  lambda: T_ANY,
    "anytype": lambda: T_ANY,
    "comptime_int": lambda: T_NUMBER(),
    "comptime_float": lambda: T_NUMBER(float=True),
}
