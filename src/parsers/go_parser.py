"""Parse Go source → unified AST using tree-sitter-go."""
...
# ---------------------------------------------------------------------------
try:
    import tree_sitter_go as _ts_lang
...
_NODE_IMPORT       = "import_declaration"
_NODE_FUNCTION     = "function_declaration"
_NODE_METHOD       = "method_declaration"
_NODE_CLASS        = "type_declaration"      # Go uses type_declaration for both struct/interface
_NODE_STRUCT       = "type_declaration"
_NODE_INTERFACE    = "type_declaration"
_NODE_VAR          = "var_declaration"
_NODE_CONST        = "const_declaration"

_STMT_RETURN       = "return_statement"
_STMT_IF           = "if_statement"
_STMT_WHILE        = "for_statement"         # Go has no while — for with only condition
_STMT_FOR          = "for_statement"
_STMT_FOR_EACH     = "for_statement"         # for x := range y {}
_STMT_ASSIGN       = "assignment_statement"
_STMT_VAR_DECL     = "short_var_declaration" # :=
...
_EXPR_FIELD        = "selector_expression"
_EXPR_INDEX        = "index_expression"
_EXPR_IDENT        = "identifier"
_EXPR_INT_LIT      = "int_literal"
_EXPR_FLOAT_LIT    = "float_literal"
_EXPR_STRING_LIT   = "interpreted_string_literal"
...
_PRIM_TYPES: Dict[str, object] = {
    "int":     lambda: T_NUMBER(64, signed=True),
    "int8":    lambda: T_NUMBER(8, signed=True),
    "int16":   lambda: T_NUMBER(16, signed=True),
    "int32":   lambda: T_NUMBER(32, signed=True),
    "int64":   lambda: T_NUMBER(64, signed=True),
    "uint":    lambda: T_NUMBER(64, signed=False),
    "uint8":   lambda: T_NUMBER(8, signed=False),
    "uint16":  lambda: T_NUMBER(16, signed=False),
    "uint32":  lambda: T_NUMBER(32, signed=False),
    "uint64":  lambda: T_NUMBER(64, signed=False),
    "float32": lambda: T_NUMBER(32, signed=True, float=True),
    "float64": lambda: T_NUMBER(64, signed=True, float=True),
    "string":  lambda: T_STRING,
    "bool":    lambda: T_BOOLEAN,
    "byte":    lambda: T_NUMBER(8, signed=False),
    "rune":    lambda: T_NUMBER(32, signed=True),
    "error":   lambda: T_NAMED("error"),
    "any":     lambda: T_ANY,
}
...
def _parse_top(node: "Node", source: str) -> "Optional[ASTNode | List[ASTNode]]":
    ...
    if t == _NODE_CONST:
        return _parse_variable(node, source, is_const=True)
    if t == "type_declaration":
        # Go type_declaration wraps struct / interface / alias
        spec = _child(node, "type_spec")
        if spec:
            type_node = _child(spec, "struct_type", "interface_type")
...
def _parse_import(node: "Node", source: str) -> "List[ImportNode]":
    ...
    for spec in node.children:
        if spec.type == "import_spec":
            path_node = spec.child_by_field_name("path") or _child(spec, "interpreted_string_literal", "raw_string_literal")
            ...
            results.append(ImportNode(id=f"import:{idx}:{module}", module=module, alias=alias))
        elif spec.type == "import_spec_list":
            for sub in spec.children:
                if sub.type == "import_spec":
                    path_node = sub.child_by_field_name("path") or _child(sub, "interpreted_string_literal")
    ...
    if not results:
        ...
        idx = _IMPORT_CTR[0]; _IMPORT_CTR[0] += 1
        results.append(ImportNode(id=f"import:{idx}:{raw}", module=raw))
    return results
